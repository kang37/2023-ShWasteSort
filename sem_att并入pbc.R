...
tpb_sem_model <- '
  ...
  APBC =~ ws_attitude + if_know_method + if_sign_sort + category_trouble + time_cost_troub

  # 结构路径
  wil_of_engage ~ SN + APBC
...
plot_data <- path_results %>%
  ...
  mutate(
    ...
    Path = case_when(
      ...
      lhs == "wil_of_engage" & rhs == "APBC" ~ "APBC → BI",
    ...
    Path_full = case_when(
      ...
      lhs == "wil_of_engage" & rhs == "APBC" ~ "APBC → Intention",
