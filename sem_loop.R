# ============================================================================
# SEM模型循环测试：测试ATT和PBC不同观测变量组合（增强版）
# ============================================================================
...
cat("SEM模型循环测试（增强版）\n")
cat(strrep("=", 80), "\n")
cat("总共需要测试的模型数:", nrow(all_model_combinations), "\n")
...
fit_single_model <- function(model_id, att_vars, pbc_vars, data, 
                             ordered_vars_all) {
  ...
  # 构建模型字符串
  model_string <- build_sem_model(att_vars, pbc_vars, sn_fixed)
  
  # 确定有序变量（只包含模型中实际使用的变量）
  model_vars <- c(att_vars, pbc_vars, sn_fixed)
  ...
  if (!is.null(fit_result)) {
    ...
    if (converged) {
      ...
      # 模型未收敛
      cat("⚠️  模型未收敛\n")
      warnings_list <- c(warnings_list, "模型未收敛 (Model did not converge)")
      fit_result_converged <- FALSE
    }
    
  } else {
    cat("❌ 模型拟合失败（fit_result为NULL）\n")
  }
...
plot_path_coefficients <- function(path_results, model_id, att_vars, pbc_vars) {
  ...
  p <- ggplot(plot_data, aes(x = year_num, y = std.all)) +
    ...
    theme(
      plot.title = element_text(
        hjust = 0.5, face = "bold", size = 10, margin = margin(b = 10)
      ),
...
ordered_vars_all <- c(
  ...
  "ws_attitude", "threat", "pr_atten", "if_no_ws_guilty", 
  "regulate_law", "regulate_commu_rule", "if_neighbor_ws", "if_family_ws", 
  "if_know_method", "if_sign_sort", "category_trouble", "time_cost_troub", 
  "wil_of_engage"
)

# 检查变量是否存在
check_vars <- c(att_candidates, pbc_candidates, sn_fixed, 
...
for (i in 1:nrow(all_model_combinations)) {
  ...
  if (result$converged) {
    ...
    plot <- plot_path_coefficients(
      result$path_results, i, att_vars, pbc_vars
    )
    all_plots[[i]] <- plot
...
print(summary_table_sorted, n = 20)
...
if (nrow(summary_table_sorted) > 0) {
  ...
  print(summary_table_sorted %>% 
          select(model_id, cfi, tli, rmsea, att_vars, pbc_vars) %>% 
          head(5))
}
